...
# Buco
    ...
    month_sales = Short_Date_Dict[Month]+"_Unit Sales"
    
    try:
        # Get retailers map
        df_bu_map = df_map
        df_bu_map['Article'] = df_bu_map['Article code'].astype(str)
        
        # Get retailer data
        df_bu_data = df_data
        df_bu_data = df_bu_data.drop(0)
        new_header = df_bu_data.columns.astype(str) +"_"+df_bu_data.iloc[0]
        df_bu_data.columns = new_header
        df_bu_data = df_bu_data[1:]
        df_bu_data = df_bu_data.rename(columns={month_sales:'Sales Qty','Unnamed: 1_BranchName':'Store Name','Unnamed: 4_Partno':'Article','Unnamed: 5_FullDesc':'Product Description'})
        column_number = [i+1 for i, x in enumerate(df_bu_data.columns) if x=='Sales Qty']
        total_amt = df_bu_data.columns[column_number[0]]
        df_bu_data = df_bu_data.rename(columns={total_amt:'Total Amt'})

        # Merge with retailer map 
        df_bu_merged = df_bu_data.merge(df_bu_map, how='left', on='Article')

        # Find missing data
        missing_model_bu = df_bu_merged['SMD product code'].isnull()
        df_bu_missing_model = df_bu_merged[missing_model_bu]
        df_missing = df_bu_missing_model[['Article','Product Description']]
        df_missing_unique = df_missing.drop_duplicates()
        st.write("The following products are missing the SMD code on the map: ")
        st.table(df_missing_unique)
...
# Takealot Marketplace
    ...
    try:
        ...
        st.markdown("**Retailer data column headings:** Order Date, TSIN, Product Title, Qty, Total Sales Value")
        st.markdown("Column headings are **case sensitive.** Please make sure they are correct")

    try:
        # Set date columns
        df_takealot_merged['Start Date'] = df_takealot_merged['Order Date']

        # Total amount column
        df_takealot_merged['Total Amt'] = df_takealot_merged['Total Sales Value']

        # Add retailer and store column
        df_takealot_merged['Forecast Group'] = 'Takealot MarketPlace'
        df_takealot_merged['Store Name'] = ''
        df_takealot_merged['Total_Stock'] = ''

        # Rename columns
        df_takealot_merged = df_takealot_merged.rename(columns={'TSIN': 'SKU No.','Qty' :'Sales Qty','Total_Stock':'SOH Qty','SMD Code':'Product Code' })
